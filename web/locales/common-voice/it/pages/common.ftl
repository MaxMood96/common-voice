# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = Afar
aat = Arvanitika
ab = Abcaso
abb = Bankon
abq = Abaza
ace = Aceh
ady = Adighè
af = Afrikaans
aii = Suret (neo-aramaico)
ajg = Adja
aln = Gheg albanese
am = Amarico
an = Aragonese
apc = Arabo levantino
ar = Arabo
arn = Mapudungun
as = Assamese
ast = Asturiano
az = Azerbaigiano
azz = Nahuatl dei monti di Puebla
ba = Baschiro
bag = Tuki
bal = Beluci
bas = Basaa
bax = Bamun
bba = Bariba
bbj = Ghomala’
bbl = Tush
bce = Mengambo
bci = Baulé
be = Bielorusso
beb = Bebele
bew = Betawi
bfd = Bafut
bfq = Badaga
bft = Balti
bg = Bulgaro
bgp = Beluci orientale
bin = Edo
bkh = Bakoko
bkm = Kom
blk = Pa'O
bm = Bambara
bn = Bengalese
bnm = Batanga
bnn = Bunun
bo = Tibetano
br = Bretone
brh = Brahui
bri = Mokpwe
brx = Bodo
bs = Bosniaco
bsh = Kateviri
bsk = Brushaski
bsy = Sabah Bisaya
btv = Bateri
bum = Bulu
bxk = Bukusu
bxr = Buriato
byv = Medumba
ca = Catalano
cak = Kaqchikel
cdo = Min orientale
ce = Ceceno
cgg = Chiga
cjk = Chokwe
ckb = Curdo centrale
cnh = Hakha Chin
co = Còrso
cpx = Heng Hua
cpy = Asheninka South Ucayali
crh = Tataro di Crimea
cs = Ceco
cut = Teutila Cuicatec
cux = Tepeuxila Cuicatec
cv = Ciuvascio
cy = Gallese
da = Danese
dag = Dagbani
dar = Dargwa
dav = Kidaw’ida
de = Tedesco
din = Dinka
dmk = Dawoodi
dml = Dameli
dru = Rukai
dsb = Basso sorabo
dua = Duala
dv = Maldiviano
dyu = Dioula
dz = Dzongkha
ebr = Ebrie
ee = Ewe
efk = Efik
eko = Ekoti
el = Greco
en = Inglese
eo = Esperanto
es = Spagnolo
esu = Yupʼik dell’Alaska centrale
et = Estone
eto = Eton
eu = Basco
ewo = Ewondo
fa = Persiano
fan = Fang
ff = Fulah
fi = Finlandese
fmp = Feʼefeʼe
fo = Faroese
fr = Francese
fub = Fulfulde dell’Adamawa
fue = Fulfulde del Borgu
fuf = Pular Guinea
fy-NL = Frisone
ga-IE = Irlandese
gaa = Ga
gcf = Creolo della Martinica
gej = Mina
ggg = Gurgula
gid = Guidar
gig = Goaria
giz = Guiziga
gjk = Kachhi
gju = Gujari
gl = Gallego
glk = Gilaki
gn = Guaraní
gom = Konkani goana
gos = Gronings
gsw = Alsaziano
gu-IN = Gujarati
guc = Wayuunaiki
gv = Manx
gwc = Gawri
gwt = Gawarbaiti
gya = Gbaya nord-occidentale
ha = Hausa
hac = Gorani
hak = Hakka
haz = Hazargi
hch = Wixárika
he = Ebraico
hem = Kihemba
hi = Hindi
hil = Ilongo
hno = Hindko settentrionale
hr = Croato
hrx = Hunsrik
hsb = Alto sorabo
ht = Creolo haitiano
hu = Ungherese
hux = Nüpode Huitoto
hy-AM = Armeno
ia = Interlingua
iba = Iban
ibb = Ibibio
id = Indonesiano
ie = Interlingue
ig = Igbo
ipk = Iñupiaq
is = Islandese
it = Italiano
izh = Izoriano
ja = Giapponese
jam = Jamaican Patois
jbo = Lojban
jgo = Ngomba
jqr = Jaqaru
jv = Giavanese
ka = Georgiano
kaa = Karakalpak
kab = Cabilo
kam = Kikamba
kbd = Cabardo
kcn = Nubi
kdh = Kotokoli
khw = Khowar
ki = Gikuyu
kjh = Khakas
kk = Kazako
kl = Kalaallisut
kln = Kalenjin
kls = Kalasha
km = Khmer
kmr = Curdo Kurmanji
kn = Kannada
knn = Konkani (Devanagari)
ko = Coreano
koo = Konzo
kpv = Komi-Zyrian
krc = Karachay-Balkar
ks = Kashmiri
ksf = Bafia
kvx = Parkari Koli
kw = Cornico
kxp = Wadiyara Koli
ky = Kirghiso
kzi = Kelabit
laj = Lango
lb = Lussemburghese
led = Lendu
leu = Kara
lg = Luganda
lij = Ligure
lke = Kenyi
lld = Ladin
ln = Lingala
lo = Lao
lrk = Loarki
lrl = Achomi
lss = Lassi
lt = Lituano
ltg = Letgallo
lth = Thur
lua = Tshiluba
luo = Dholuo
lv = Lettone
lzz = Laz
mai = Maithili
mau = Huautla Mazatec
mbf = Baba malese
mbo = Mbo
mcf = Matses
mcn = Massa
mcx = Mpiemo
mdd = Mbum
mdf = Mokša
meh = Mixteco Yucuhiti
mel = Melanau
mfe = Kreol Morisien
mg = Malgascio
mgg = Kunabembe
mhk = Mungaka
mhr = Mari orientale
mk = Macedone
mki = Dhatki
ml = Malayalam
mmc = Michoacán Mazahua
mn = Mongolo
mni = Meitei Lon
mnw = Mon
mos = More
mqh = Mixteco tlazoyaltepec
mr = Marathi
mrh = Mara
mrj = Mari occidentale
ms = Malese
mse = Moussey
msi = Sabah malese
mt = Maltese
mua = Mundang
mug = Musgum
mui = Musi
mve = Marwari pachistano
mvy = Indus Kohistani
mxu = Mada
my = Birmano
myv = Erza
mzn = Mazandarani
nan-tw = Taiwanese (Minnan)
nb-NO = Bokmål (Norvegia)
ncx = Central Puebla Nahuatl
nd = IsiNdebele del nord
ne-NP = Nepalese
new = Newari
nhe = Huasteca Nahuatl orientale
nhi = Nahuatl della Sierra di Puebla occidentale
nia = Nias
nl = Olandese
nla = Ngombale
nlv = Orizaba Nahuatl
nmg = Kwasio
nmz = Losso
nn-NO = Norvegese Nynorsk
nnh = Ngiembon
nqo = NKo
nr = IsiNdebele del sud
nso = Sotho del nord
ny = Cinyanja
nyn = Nyankole
nyu = Nyungwe
oc = Occitano
odk = Oadki
om = Afaan Ormoo
or = Oriya
oru = Ormuri
os = Ossetico
pa-IN = Punjabi
pap = Papiamento
pap-AW = Papiamento (Aruba)
pau = Palauan
pcd = Piccardo
pcm = Pidgin nigeriano inglese
pez = Penan orientale
phl = Palula
phr = Pahari-Pothwari
pl = Polacco
plk = Kohistani Shina
pne = Penan occidentale
prq = Asheninka Perene
ps = Pashtu
pt = Portoghese
pua = Purepecha delle Highlands occidentali
pwn = Paiwan
quc = Quiché
qug = Quichua Chimborazo Highland
qup = Pastaza Quechua meridionale
qur = Quechua Yanahuanca
qus = Quechua Santiago del Estero
qux = Quechua Yauyos
quy = Quechua Chanka
qva = Quechua Ambo-Pasco
qvi = Kichwa
qvj = Loja Highland Kichwa
qvl = Quechua Cajatambo
qwa = Quechua Corongo Ancash
qws = Quechua Sihuas Ancash
qxa = Quechua Chiquián
qxp = Puno Quechua
qxq = Qashqai
qxt = Quechua Pasco Santa Ana de Tusi
qxu = Quechua Arequipa-La Unión
qxw = Quechua Jauja Wanka
rif = Tarifit
rki = Rakhine
rm-sursilv = Romancio sottosilvano
rm-vallader = Romancio Vallader
rn = Kirundi
ro = Rumeno
rof = Kirombo
ru = Russo
ruc = Ruuli
rup = Aromanian
rw = Kinyarwanda
rwm = Amba
sah = Sacha
sat = Santali (Ol Chiki)
sba = Ngambay
sbn = Sindhi Bhili
sc = Sardo
scl = Shina
scn = Siciliano
sco = Scots
sd = Sindhi
sdh = Curdo meridionale
sdo = Serian Bidayuh
seh = Sena
sei = Seri
sg = Sango
shi = Shilha
shn = Shan
si = Singalese
sk = Slovacco
skr = Saraiki
sl = Sloveno
sn = Shona
snk = Soninke
snv = Sa’ban
so = Somalo
sq = Albanese
sr = Serbo
ss = Swati
ssi = Sansi
st = Sesotho
su = Sundanese
sv-SE = Svedese
sva = Svan
sw = Swahili
swb = Shimaore
syr = Siriaco
szy = Sakizaya
ta = Tamil
tar = Central Tarahumara
tay = Atayal
te = Telugu
teg = Teke
tg = Tagico
th = Tailandese
ti = Tigrino
tig = Tigre
tiv = Tiv
tk = Turkmeno
tl = Tagalog
tli = Tlingit
tn = Tswana
tob = Toba Qom
tok = Toki Pona
top = Papantla Totonac
tr = Turco
trv = Seediq
trw = Torwali
ts = Xitsonga
tt = Tataro
ttj = Rutoro
tui = Tupuri
tum = Tumbuka
tuq = Tedaga
tvu = Tunen
tw = Twi
ty = Tahitiano
tyv = Tuvano
uby = Ubykh
udl = Ouldémé
udm = Udmurto
ug = Uiguro
uk = Ucraino
ukv = Kuku
ur = Urdu
ush = Ushojo
usp = Uspanteko
uz = Uzbeco
var = Huarijio
ve = Tshivenda
vec = Veneto
vi = Vietnamita
vls = Fiammingo occidentale
vmw = Emakhuwa
vot = Votico
wbl = Wakhi
wep = Vestfaliano
wes = Pidgin del Camerun
wo = Wolof
xdq = Kaitag
xh = Xhosa
xhe = Ketrani
xka = Kalkoti
xkl = Kenyah
xmf = Mingrelian
xsm = Kasem
yaq = Yaqui
yav = Nuasue
ydg = Yadgha
yi = Yiddish
yo = Yoruba
yue = Cantonese
zgh = Berbero
zh-CN = Cinese (Cina)
zh-HK = Cinese (Hong Kong)
zh-TW = Cinese (Taiwan)
zoc = Copainalá Zoque
zu = Zulu
zza = Zaza

## Layout

profile = Profilo
help = Supporto
contact = Contatti
privacy = Privacy
terms = Condizioni
cookies = Cookie
faq = FAQ
documentation = Documentazione
content-license-text = Contenuto disponibile sotto <licenseLink>licenza Creative Commons</licenseLink>
share-title = Aiutaci a trovare altri donatori di voce!
share-text = Aiutaci a insegnare alle macchine come le persone parlano nella vita reale, dona la tua voce su { $link }
link-copied = Link copiato
back-top = Torna in alto
logout = Disconnetti
donate = Fai una donazione
stayintouch = Mozilla sta radunando una comunità di persone interessate alle tecnologie vocali. Ci piacerebbe rimanere in contatto con te con aggiornamenti e nuove fonti di dati, ma anche per conoscere come usi i dati di Common Voice.
privacy-info = Le tue informazioni personali sono al sicuro con noi. Per i dettagli leggi l’<privacyLink>informativa sulla privacy</privacyLink>.
return-to-cv = Torna a Common Voice
email-input =
    .label = Email
loading = Caricamento in corso…
email-opt-in-info = Mi piacerebbe ricevere email come promemoria degli obiettivi, aggiornamenti sui miei progressi e newsletter riguardo Common Voice.
email-opt-in-info-title = Iscriviti alla mailing list di Common Voice
email-opt-in-info-sub-with-challenge = Ricevi email come promemoria di sfide e obiettivi, aggiornamenti sui progressi e newsletter di Common Voice.
email-opt-in-privacy-v2 = Scegliendo di ricevere le email, accetti che Mozilla utilizzi queste informazioni secondo quanto riportato nell’<privacyLink>informativa sulla privacy</privacyLink>.
indicates-required = * indica un campo obbligatorio
not-available-abbreviation = N.D.
# Text shown on a button that can be clicked to copy text to the clipboard
copy = Copia
# Text shown on a button that can be clicked to close a modal or panel
close = Chiudi
# Text shown on a button that can be clicked to cancel an action
cancel = Annulla
# Text shown on a button that can be clicked to delete an item
delete = Elimina

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>Abbiamo bisogno del tuo</mark><br/><mark>aiuto!</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = Ospitare i dataset e migliorare la piattaforma per le oltre 100 comunità linguistiche che fanno affidamento su ciò che facciamo costa quasi un milione di dollari all’anno. <strong>Se apprezzi la possibilità di accedere a dati aperti e inclusivi, fai una donazione oggi stesso!</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = <mark>Utilizzi Common Voice per</mark><br/><mark>le tue ricerche?</mark>
languages-donate-banner-cta-explanation = Common Voice è finanziato da donazioni e sovvenzioni. Amiamo collaborare con accademici, società civile e ricercatori del settore. Common Voice è gratuito, ma sostenere parte dei costi della piattaforma e dell’hosting attraverso proposte di sovvenzione è davvero utile.
localization-select =
    .label = Scegli lingua/localizzazione

## MDC Announcement

# Main text for MDC announcement. Text wrapped in <strong></strong> will be rendered with bold font
announcement-mdc-text = <strong>I nuovi dataset di Common Voice</strong> sono ora disponibili per il download esclusivamente attraverso la nostra piattaforma gemella, Mozilla Data Collective. Iscriviti per accedere a oltre 300 dataset globali, realizzati da e per la comunità.
# Button text for MDC announcement
announcement-mdc-button-text = Unisciti a Mozilla Data Collective
# Aria text for button which opens MDC on a new page
announcement-mdc-button-aria-text = Si apre in una nuova scheda

## Release Delay Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-delay = Cari membri della community, stiamo lavorando al prossimo rilascio. Prevediamo che nei prossimi giorni sarà distribuito gradualmente su <mdcLink>Mozilla Data Collective</mdcLink>.

## Release Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release = Cari membri della comunità, la nuova versione è ora completamente disponibile su <mdcLink>Mozilla Data Collective</mdcLink>. Partecipa per accedere a più di 500 dataset globali, creati dalla comunità e per la comunità.

## Pre-Release Announcement (dates TBD)

# Text wrapped in <cvDatasheetsLink></cvDatasheetsLink> will be rendered as an external link to the cv-datasheets GitHub repo
announcement-pre-release = Cari membri della community, il momento del rilascio si avvicina. Vi invitiamo a validare e trascrivere, e a visitare anche <cvDatasheetsLink>cv-datasheets</cvDatasheetsLink> per aggiornare il vostro datasheet.

## Release Commencing

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-commencing = Gentile comunità, abbiamo raggiunto la data di chiusura per la versione e stiamo preparando i dataset. La nuova versione sarà disponibile su <mdcLink>Mozilla Data Collective</mdcLink> tra pochi giorni.

## Downtime Announcements

# Generic scheduled downtime for next Sunday early hours
announcement-scheduled-downtime = Cara comunità, per ottimizzare i nostri sistemi programmiamo brevi interventi di manutenzione, che in genere si concludono nel giro di poche ore. Il prossimo è previsto nelle prime ore di domenica. In caso di problemi, contattateci su Matrix.

## Common Language/Dataset Selector & SearchBox Related

# Generic search box defaults
searchbox-default-label = Inizia a digitare per cercare
searchbox-default-placeholder = Cerca…
searchbox-default-aria-label = Cerca
dropdown-no-results = Nessuna corrispondenza trovata
# Common Voice Dataset=Language search box
dataset-searchbox-label = Cambia sia la lingua di visualizzazione che quella del dataset
dataset-searchbox-placeholder = Cerca una lingua…
dataset-searchbox-aria-label = Cerca una lingua
dataset-search-no-results = Nessuna lingua corrispondente trovata
request-new-language = Richiedi nuova lingua
